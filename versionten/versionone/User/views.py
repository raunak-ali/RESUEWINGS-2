...
def AboutView(request):
    ...
    return render(request=request, template_name="projectfinal/About.html", context={})
...
def home(request):
    if request.method == "POST":
        ...
        #R=inspect.getmembers(Profile_form)
        #print(Profile_form)
        #print(R)
        if Profile_form.is_valid():
            ...
            PF.Paired_with_ID='0'
            R=PF.Name
            if(Profiles.objects.filter(Name=R)):
                print(PF.Name)
                messages.error(request, 'Please chooose a different name for  your submission as This name is already in use')
            else:
                PF.save()
...
def  ContactView(request):
    return render(request=request, template_name="projectfinal/contact.html", context={})
...
def  UPDATE_DB_VIEW(Request,Volunteer):
    ...
    if(v<1 or n<1):
        ...
        for r in range(0,n):
            ...
            RR.Pair_Found=True
            VOL.Pair_Found=True
            RR.Paired_with=VOL.User.username
            RR.Paired_with_ID=VOL.id
            VOL.Paired_with=RR.User.username
